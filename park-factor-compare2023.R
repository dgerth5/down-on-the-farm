...
milbpf_wlgs_2023 <- read_csv("milb_park_effects23-wpf.csv")
...
fnl_milb_pf <- cbind(milbpf_wlgs_2023, empy_df_single, empy_df_double, empy_df_triple, empy_df_home_run)
...
gt(bot_hr) %>%
  tab_header(title = md("**Players Who Were Hurt The Most**"),
             subtitle = md("Season: 2023, Min 200 PA")) %>%
...
write_csv(leader, "adj_leaderboard23.csv")

# parks 
# run
top_hr <- milbpf_wlgs_2023 %>%
  mutate(league_lvl = paste0(tools::toTitleCase(league), " (", toupper(level), ")")) %>%
  select(team, league_lvl, home_run_pf) %>%
  arrange(-home_run_pf) %>%
  head(5)

bot_hr <- milbpf_wlgs_2023 %>%
  mutate(league_lvl = paste0(tools::toTitleCase(league), " (", toupper(level), ")")) %>%
  select(team, league_lvl, home_run_pf) %>%
  arrange(home_run_pf) %>%
  head(5) 

colnames(bot_hr) <- paste0(colnames(bot_hr),"2")

cb <- cbind(top_hr, bot_hr)

hr_pk <- cb %>%
  gt() %>%
  tab_header(title = md("**Top and Bottom 5 Home Run Parks**"),
             subtitle =  md("Season: 2023")) %>%
  gt_add_divider("home_run_pf") %>%
  fmt_number(c("home_run_pf", "home_run_pf2"), decimals = 0) %>%
  cols_label(team = "Team",
             ...
             home_run_pf2 = "PF")
gtsave(hr_pk, "hr_pk23.png")


# player

lb23 <- read_csv("adj_leaderboard23.csv")

lb23_2 <- lb23 %>%
  filter(tot_pa > 400) %>%
  mutate(ops = obp + slg,
         adj_ops = adj_obp + adj_slg) %>%
  select(Name, tot_pa, ops, adj_ops, diff_ops)
...
colnames(bot_lb23) <- paste0(colnames(bot_lb23),"2")

cblb23 <- cbind(top_lb23, bot_lb23)

lb23_photo <- cblb23 %>%
  gt() %>%
  tab_header(title = md("**Who Was Most Affected by Their Ballpark?**"),
             subtitle =  md("Season: 2023. Min 400 PA")) %>%
  gt_add_divider("diff_ops") %>%
  cols_label(tot_pa = "PA",
             ops = "OPS",
             adj_ops = "adjOPS",
             diff_ops = "Diff",
             Name2 = "Name",
