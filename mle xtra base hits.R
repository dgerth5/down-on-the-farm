...
clean_fn <- function(df, yr, id_high_lvl, id_lo_lvl){
  ...
  id1 <- df %>% 
    ...
    select(player_id, player_full_name, league_name, tot_pa, tot_doubles, tot_triples)
  ...
  means <- rbind(id1, id2) %>%
    mutate(xbh_per = (tot_doubles + tot_triples) / tot_pa) %>%
    filter(tot_pa > 50) %>%
    group_by(league_name) %>%
    summarise(mean_xbh = mean(xbh_per, na.rm = TRUE),
              sd_xbh = sd(xbh_per, na.rm = TRUE)) 
  
  
  df1 <- left_join(id2, id1, by = "player_id") %>%
    mutate(xbh_perlo = (tot_doubles.x + tot_triples.x) / tot_pa.x, 
           xbh_perhi = (tot_doubles.y + tot_triples.y) / tot_pa.y,
    ...
    mutate(xbh_perhi_z = if_else(is.na(xbh_perhi)==TRUE, NA, (xbh_perhi - mean_xbh.y) / sd_xbh.y),
           xbh_perlo_z = if_else(is.na(xbh_perlo)==TRUE, NA, (xbh_perlo - mean_xbh.x) / sd_xbh.x)) %>%
    filter(play_next_lvl == 1) %>%
    filter(tot_pa.x > 100) %>% filter(tot_pa.y > 100) %>% ungroup()
...
combined_df_mlb <- bind_rows(s18mlb, s19mlb, s21mlb, s22mlb, s23mlb) %>%
  mutate(xbh_change_per = xbh_perhi_z - xbh_perlo_z)
...
tbl_df <- data.frame(Lvl = c("Triple A -> MLB", 
                             ...
                             "High-A -> Double-A",
                             "Single-A -> High-A"),
                     PercentIncrease = c(round(mean(combined_df_mlb$xbh_change_per),2), 
                                         round(mean(combined_df_trip$xbh_change_per),2), 
                                         round(mean(combined_df_dbl$xbh_change_per),2), 
                                         round(mean(combined_df_hi_a$xbh_change_per),2)))
...
get_z_score_dat <- function(df, yr){
  ...
  id1 <- df %>% 
    filter(year == yr) %>%
    select(player_id, player_full_name, league_name, sport_id, tot_pa, tot_doubles, tot_triples) %>%
    mutate(xbh_per = (tot_doubles + tot_triples) / tot_pa)
  
  means <- id1 %>%
    filter(tot_pa > 50) %>%
    group_by(sport_id) %>%
    summarise(mean_bb = mean(xbh_per, na.rm = TRUE),
              sd_bb = sd(xbh_per, na.rm = TRUE)) }
