...
milb_stats <- map_df(2023:2024, ~ get_minor_lg_stats(season = .x))
...
clean_lookup <- function(df) {
  ...
  lookup = df %>% 
    ...
    mutate(age2024 = lubridate::year(lubridate::as.period(lubridate::interval(start = lubridate::mdy(dob), end = lubridate::mdy("03-30-2024")))))
...
milb_stats3 <- milb_stats2 %>%
  ...
  select(player_id, player_name, season, norm_sba, norm_tr, norm_sgr, tot_pa, age2024) %>%
  group_by(player_id) %>%
  mutate(lag_sba = lag(norm_sba),
         lag_tr = lag(norm_tr),
         lag_s2gbo = lag(norm_sgr),
...
write_csv(just_milb, "milb-sprint-speed2024.csv")
...
table1 <- just_milb %>%
  ungroup() %>%
  slice(1:10) %>%
  mutate(team = c("SF", "DET", "BAL", "TB", "NYM", "COL", "TB", "STL", "COL", "BAL"),
...
gtsave(table1, "sprint_speed_milb2024.png")
