...
get_minor_lg_stats <- function(season){
  ...
  df2 <- df %>%
    ...
    summarise(tot_bf = sum(batters_faced),
...
clean_fn <- function(df, yr, id_high_lvl, id_lo_lvl){
  ...
  id1 <- df %>% 
    ...
    select(player_id, player_full_name, league_name, tot_bf, tot_k)
  ...
  df1 <- left_join(id2, id1, by = "player_id") %>%
    mutate(k_perlo = tot_k.x / tot_bf.x, 
           k_perhi = tot_k.y / tot_bf.y,
           play_next_lvl = if_else(is.na(tot_bf.y)==TRUE, 0, 1)) %>%
    ...
    filter(tot_bf.x > 50) %>% filter(tot_bf.y > 50)
...
combined_df_mlb <- bind_rows(s18mlb, s19mlb, s21mlb, s22mlb, s23mlb)  %>%
  mutate(k_change_per = k_perhi_z - k_perlo_z)
...
tbl_df <- data.frame(Lvl = c("Triple A -> MLB", 
                             ...
                             "High-A -> Double-A",
                             "Single-A -> High-A"),
                     PercentIncrease = c(round(mean(combined_df_mlb$k_change_per),2), 
                                         round(mean(combined_df_trip$k_change_per),2), 
                                         round(mean(combined_df_dbl$k_change_per),2), 
                                         round(mean(combined_df_hi_a$k_change_per),2)))
...
get_z_score_dat <- function(df, yr){
  ...
  id1 <- df %>% 
    filter(year == yr) %>%
    select(player_id, player_full_name, league_name, sport_id, tot_bf, tot_k) %>%
    mutate(k_per = tot_k / tot_bf)
