...
just2 <- just_pitches %>% 
  select(game_date, matchup.pitcher.id, matchup.batter.id, details.type.code, pitchData.startSpeed, about.atBatIndex, pitchNumber, pitchData.breaks.spinRate) %>%
...
statcast2 <- statcast22 %>%
  select(game_date, pitcher, batter, pitch_type, release_speed, at_bat_number, pitch_number, release_spin_rate) %>%
...
master2 <- master %>%
  select(release_spin_rate, pitchData.breaks.spinRate, release_speed)

mod <- lm(release_spin_rate ~ pitchData.breaks.spinRate + release_speed, data = master2)
summary(mod)