...
dates <- data.frame(day = rep(seq(as.Date("2023-04-06"),as.Date("2023-09-30"), by = "days"), times = 1))
...
pbp2 <- milb_pbp %>% 
  ...
  select(details.description, matchup.pitchHand.code, matchup.batter.fullName, pitchData.zone, count.strikes.start, count.balls.start, details.type.code,
         ...
         pitchData.coordinates.x, pitchData.coordinates.y, hitData.launchAngle, hitData.launchSpeed) %>%
...
train_pool <- catboost.load_pool(data = pbp2[,-c(1:4,6:7,10,12,16:19)],
                                 label = pbp2$se)
...
croe <- pbp2 %>%
  ...
  summarise(xchase = mean(predicted_probs),
            chase = mean(se),
            n = n()) %>%
  mutate(croe = round(xchase - chase,3),
         croe_pr = 100*percent_rank(croe)) # lower / better
...
merged_df <- croe %>%
  ...
  select(matchup.batter.fullName, croe, croe_pr, ev90, ev90_per, dhh, dhh_per, ss, ss_per, whiff_rate, wh_per, swing_rate, sw_per, pitches_seen) %>%
  drop_na()

write_csv(merged_df, "triple-a-statcast-smry9-30.csv")
...
md2 <- merged_df %>%
  ...
  filter(croe_pr >= 50 & ev90_per >= 50 & dhh_per >= 50 & ss_per >= 50 & wh_per >= 50)

library(gt)
library(readr)

merged_df <- read_csv("triple-a-statcast-smry9-30.csv")

merged_df %>% 
  filter(matchup.batter.fullName == "Heston Kjerstad") %>%
  select(-pitches_seen) %>%
  gt() %>%
  tab_header(title = md("**Heston Kjerstad**"),
             subtitle = md("CROE number relative to Triple A competition")) %>%
  fmt_number(columns = c("croe","ev90","dhh","ss","whiff_rate", "swing_rate")) %>%
  fmt_number(columns = c("croe_pr","ev90_per","dhh_per","ss_per","wh_per", "sw_per"), decimals = 0) %>%
  data_color(
    columns = c("croe_pr", "ev90_per", "dhh_per", "ss_per", "wh_per"),
  ...
  data_color(
    columns = c("sw_per"),
    colors = scales::col_numeric(
      palette = c("red", "white", "green", "white", "red"),
      domain = c(0, 100))) %>%
  cols_label(matchup.batter.fullName = "Name")
             # croe = "CROE", croe_pr = "CROE Per",
             # ev90 = "EV90", ev90_per = "EV90 Per",
             # dhh = "DHH%", dhh_per = "DHH Per",
             # ss = "SS%", ss_per = "SS Per",
             # whiff_rate = "Whiff%", wh_per = "Whiff% Per",
             # swing_rate = "Swing%", sw_per = "Swing% Per") 
