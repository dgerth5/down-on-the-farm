...
clean_fn <- function(df, yr, id_high_lvl, id_lo_lvl){
  ...
  id1 <- df %>% 
    ...
    select(player_id, player_full_name, league_name, tot_pa, tot_hr)
  ...
  means <- rbind(id1, id2) %>%
    mutate(hr_per = tot_hr / tot_pa) %>%
    filter(tot_pa > 50) %>%
    group_by(league_name) %>%
    summarise(mean_hr = mean(hr_per, na.rm = TRUE),
              sd_hr = sd(hr_per, na.rm = TRUE)) 
  
  df1 <- left_join(id2, id1, by = "player_id") %>%
    mutate(hr_perlo = tot_hr.x / tot_pa.x, 
           hr_perhi = tot_hr.y / tot_pa.y,
    ...
    mutate(hr_perhi_z = if_else(is.na(hr_perhi)==TRUE, NA, (hr_perhi - mean_hr.y) / sd_hr.y),
           hr_perlo_z = if_else(is.na(hr_perlo)==TRUE, NA, (hr_perlo - mean_hr.x) / sd_hr.x)) %>%
...
combined_df_mlb <- bind_rows(s18mlb, s19mlb, s21mlb, s22mlb, s23mlb) %>%
  mutate(hr_change_per = hr_perhi_z - hr_perlo_z)
...
tbl_df <- data.frame(Lvl = c("Triple A -> MLB", 
                             ...
                             "High-A -> Double-A",
                             "Single-A -> High-A"),
                     PercentIncrease = c(round(mean(combined_df_mlb$hr_change_per),2), 
                                         round(mean(combined_df_trip$hr_change_per),2), 
                                         round(mean(combined_df_dbl$hr_change_per),2), 
                                         round(mean(combined_df_hi_a$hr_change_per),2)))
...
get_z_score_dat <- function(df, yr){
  ...
  id1 <- df %>% 
    filter(year == yr) %>%
    select(player_id, player_full_name, league_name, sport_id, tot_pa, tot_hr) %>%
    mutate(hr_per = tot_hr / tot_pa)
