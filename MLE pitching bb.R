...
clean_fn <- function(df, yr, id_high_lvl, id_lo_lvl){
  ...
  id1 <- df %>% 
    ...
    select(player_id, player_full_name, league_name, tot_bf, tot_bb)
  ...
  df1 <- left_join(id2, id1, by = "player_id") %>%
    mutate(bb_perlo = tot_bb.x / tot_bf.x, 
           bb_perhi = tot_bb.y / tot_bf.y,
...
get_z_score_dat <- function(df, yr){
  ...
  id1 <- df %>% 
    filter(year == yr) %>%
    select(player_id, player_full_name, league_name, sport_id, tot_bf, tot_bb) %>%
    mutate(bb_per = tot_bb / tot_bf)
